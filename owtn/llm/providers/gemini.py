...
def query_gemini(
    ...
    **kwargs,
    ...
    if output_model is not None:
        # Instructor from_provider client.
        messages = [{"role": "system", "content": system_msg}, *new_msg_history]
        content, raw_response = client.create_with_completion(
            model=model,
            messages=messages,
            response_model=output_model,
        )
        new_msg_history.append({"role": "assistant", "content": str(content)})
        cost_results = get_gemini_costs(raw_response, model)
        return QueryResult(
            content=content,
            msg=msg,
...
async def query_gemini_async(
    ...
    **kwargs,
    ...
    if output_model is not None:
        # Instructor from_provider async client.
        messages = [{"role": "system", "content": system_msg}, *new_msg_history]
        content, raw_response = await client.create_with_completion(
            model=model,
            messages=messages,
            response_model=output_model,
        )
